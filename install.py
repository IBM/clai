...
# pylint: disable=no-name-in-module,import-error,too-many-statements, broad-except
import json
import os
import re
import ssl
...
def parse_args():
    ...
    parser.add_argument(
        "--port", help="port listen server", type=int, default=8010, dest="port"
    )
    ...
    if not is_root_user(args):
        if not args.user_install:
            print_error(
                "You need root privileges for the system wide installation process."
            )
            sys.exit(1)
...
def install_plugins_dependencies(path, plugin, user_install):
    ...
    result = os.system(
        f"{path}/fileExist.sh {plugin} {path} " f'{"--user" if user_install else ""}'
    )
        ...
        for plugin in plugins:
            ...
            if default or z_default:
                installed = install_plugins_dependencies(
                    bin_path, plugin.pkg_name, user_install
                )
                if installed:
...
def append_setup_to_file(rc_path, bin_path, port):
    ...
    append_to_file(
        rc_path,
        "\n[[ -f %s/bash-preexec.sh ]] && source %s/bash-preexec.sh"
        % (bin_path, bin_path),
    )
    append_to_file(
        rc_path,
