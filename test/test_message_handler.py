...
ALL_PLUGINS = [
    AgentDescriptor(pkg_name="demo_agent", name="demo_agent"),
...
def mock_executor():
    with mock.patch(
        "clai.server.agent_runner.agent_executor", MockExecutor()
    ) as _fixture:
        yield _fixture
...
def test_should_return_the_list_of_plugins_with_default_selected_when_the_server_received_plugins_no_selected(
    ...
    mocker.patch.object(
        AgentDatasource, "get_instances", return_value=[mock_agent], autospec=True
    )
    mocker.patch.object(
        ConfigStorage, "read_config", return_value=NO_SELECTED, autospec=True
    )
    mocker.patch.object(
        AgentDatasource, "all_plugins", return_value=ALL_PLUGINS, autospec=True
    )
...
def test_should_return_the_list_of_plugins_with_selected_when_the_server_received_plugins(
    mocker,
):
    mock_agent = create_mock_agent()
    mocker.patch.object(
    ...
    mocker.patch.object(
        ConfigStorage, "read_all_user_config", return_value=None, autospec=True
    )
    mocker.patch.object(
        ConfigStorage,
        "read_config",
        return_value=PluginConfig(
            selected=[agent_selected], default_orchestrator="max_orchestrator"
        ),
        autospec=True,
    )
    message_handler = MessageHandler(ServerStatusDatasource(), AgentDatasource())
...
def test_should_return_the_list_without_any_selected_plugin_when_default_doesnt_exist(
    ...
    mocker.patch.object(
        ConfigStorage,
        "read_config",
        return_value=PluginConfig(default="", default_orchestrator="max_orchestrator"),
        autospec=True,
    )
    message_handler = MessageHandler(ServerStatusDatasource(), AgentDatasource())
...
def test_should_return_the_install_command_when_the_new_plugin_is_not_installed_yet(
    ...
    mocker.patch.object(
        ...
        "read_config",
        return_value=PluginConfig(
            selected=["nlc2cmd"], default_orchestrator="max_orchestrator"
        ),
        autospec=True,
    )
...
def test_should_return_the_list_with_the_new_selected_values_if_exists_and_is_installed(
    ...
    mocker.patch.object(
        ...
        "read_config",
        return_value=PluginConfig(
            selected=["demo_agent"], default_orchestrator="max_orchestrator"
        ),
        autospec=True,
    )
    mocker.patch.object(ConfigStorage, "store_config", return_value=None, autospec=True)
...
def test_should_return_the_action_from_selected_agent_when_the_command_goes_to_the_agent_and_threshold_is_ok(
    mocker,
):
    mock_agent = create_mock_agent()
    mocker.patch.object(
...
# @patch('clai.server.agent_executor.thread_executor', MockExecutor())
def test_should_return_the_suggestion_from_agent_ignoring_confidence_if_is_clai_command(
    mocker,
):
    mock_agent = create_mock_agent()
    mocker.patch.object(
