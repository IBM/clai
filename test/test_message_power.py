...
def test_should_not_change_power_variable_when_active_power_mode_and_it_already_active(
    mocker,
):
    mock_agent = create_mock_agent()
    mocker.patch.object(
    ...
    assert (
        action.description
        == "You have the auto mode already enable, use clai manual to deactivate it"
    )
    assert message_handler.server_status_datasource.is_power()
...
def test_should_not_change_power_variable_when_active_power_mode_and_it_already_disable(
    ...
    assert (
        action.description
        == "You have manual mode already enable, use clai auto to activate it"
    )
    assert not message_handler.server_status_datasource.is_power()
