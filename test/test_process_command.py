...
from clai.process_command import (
    process_command_from_user,
...
SUGGESTED_ACTION = Action(
    origin_command="command", suggested_command="suggested_command"
)
EXECUTABLE_ACTION = Action(
    origin_command="command", suggested_command="suggested_command", execute=True
)
...
def test_should_return_a_valid_action_when_socket_crash(mocker):
    mocker.patch.object(
        SocketClientConnector, "send", side_effect=socket.error(), autospec=True
    )
...
def test_should_return_the_original_command_when_read_throw_exception(mocker):
    mocker.patch.object(
        ClientConnector, "send", side_effect=socket.error(), autospec=True
    )

    command_to_execute, _ = process_command_from_user(
        ANY_ID, ANY_USER, ANY_INPUT_COMMAND
    )
...
def test_should_print_the_suggested_dialog_when_suggested_is_different(mocker):
    ...
    mocker.patch.object(
        ClaiClient, "send", return_value=SUGGESTED_ACTION, autospec=True
    )
...
def test_should_not_print_the_suggested_dialog_when_suggested_is_different_and_execute_is_enable(
    ...
    spy_print(mocker)
    mocker.patch.object(
        ClaiClient, "send", return_value=EXECUTABLE_ACTION, autospec=True
    )
...
def test_should_not_print_the_suggested_dialog_when_suggested_command_is_empty(mocker):
    spy_print(mocker)
    empty_suggested_command = Action(
        origin_command=ANY_INPUT_COMMAND, suggested_command=""
    )
    mocker.patch.object(
        ClaiClient, "send", return_value=empty_suggested_command, autospec=True
    )
...
def test_should_return_the_start_server_command_when_received_clai_start(mocker):
    with pytest.raises(SystemExit):
        mock_override_last_command = mocker.patch(
            "clai.process_command.override_last_command"
        )

        process_command(ANY_ID, ANY_USER, COMMAND_START_SERVER)

        mock_override_last_command.assert_called_once_with(
            f"\n{START_SERVER_COMMAND_TO_EXECUTE}\n"
        )
