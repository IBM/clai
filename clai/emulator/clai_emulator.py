...
class ClaiEmulator:
    def __init__(self, emulator_docker_bridge: EmulatorDockerBridge):
        self.presenter = EmulatorPresenter(
            emulator_docker_bridge,
    ...
    def launch(self):
        ...
        self.root.protocol(
            "WM_DELETE_WINDOW", lambda root=self.root: self.on_closing(root)
        )
    ...
    def on_skills_ready(self, skills_as_array: List[str]):
        ...
        for skill in skills_as_array[1:-1]:
            ...
            self.selected_skills_dropmenu["menu"].add_command(
                label=name_skill, command=tk._setit(self.selected_skills, name_skill)
            )

            if active:
                self.presenter.current_active_skill = self.extract_skill_name(
                    name_skill
                )[0]
    ...
    def add_detail_label(
        self, parent: ttk.Frame, title: str, text_value: str, side: str
    ):
        row = ttk.Frame(parent)
        title_label = ttk.Label(
            parent, text=title, font=self.bold_font, anchor="n", padding=(10, 0, 0, 0)
        )
    ...
    def add_row(self, response: str, info: InfoDebug):
        ...
        toggled_frame = ToggledFrame(
            self.frame, text=response, relief=tk.RAISED, borderwidth=1
        )
        ...
        self.add_detail_label(
            second_row,
            "Description:",
            self.remove_emoji(info.action_suggested.description),
            tk.LEFT,
        )
        self.add_detail_label(
            second_row, "Confidence:", f"{info.action_suggested.confidence}", tk.RIGHT
        )
        self.add_detail_label(
            second_row, "Force:", f"{info.action_suggested.execute}", tk.RIGHT
        )
        ...
        self.add_detail_label(
            third_row,
            "Agent:",
            text_value=info.action_suggested.agent_owner,
            side=tk.LEFT,
        )
        self.add_detail_label(
            third_row,
            "Sugestion:",
            text_value=info.action_suggested.suggested_command,
            side=tk.RIGHT,
        )
        self.add_detail_label(
            third_row, "Applied:", text_value=f"{info.already_processed}", side=tk.RIGHT
        )

        fourth_row = ttk.Frame(toggled_frame.sub_frame)
        fourth_row.pack(fill="x", expand=True)
        process_text = ",".join(
            list(map(lambda process: process.name, info.processes.last_processes))
        )
        self.add_detail_label(
            fourth_row, "Processes:", text_value=f"{process_text}", side=tk.LEFT
        )
        ...
        self.add_detail_label(
            fifth_row,
            "Description:",
            text_value=f"{self.remove_emoji(post_description)}",
            side=tk.LEFT,
        )
        self.add_detail_label(
            fifth_row, "Confidence:", text_value=f"{post_confidence}", side=tk.RIGHT
        )
    ...
    def add_send_command_box(self, root):
        ...
        send_button = tk.Button(
            button_bar_frame, padx=10, text="\u2713", command=self.on_send_click
        )
    ...
    def add_play_button(self, toolbar):
        ...
        self.run_button = ttk.Button(
            toolbar, image=self.run_image, command=self.on_run_click
        )
    ...
    def add_refresh_button(self, toolbar):
        ...
        refresh_button = ttk.Button(
            toolbar, image=self.refresh_image, command=self.on_refresh_click
        )
    ...
    def add_log_button(self, toolbar):
        ...
        log_button = ttk.Button(
            toolbar, image=self.log_image, command=self.open_log_window
        )
    ...
    def add_list_commands(self, root):
        ...
        canvas.bind(
            "<Configure>",
            lambda event, canvas=canvas: self.canvas_resize(event, canvas),
        )
        self.frame.bind(
            "<Configure>", lambda event, canvas=canvas: self.on_configure(canvas)
        )
        self.frame.bind_all(
            "<MouseWheel>",
            lambda event, canvas=canvas: canvas.yview_scroll(-1 * event.delta, "units"),
        )
        self.canvas = canvas
    ...
    def extract_skill_name(skill):
        ...
        return (
            skill.replace("(Installed)", "").replace("(Not Installed)", "").strip(),
            installed,
        )
    ...
    def remove_emoji(description):
        ...
        char_list = [
            ...
            if ord(description[j]) in range(65536)
        ]
        description = ""
