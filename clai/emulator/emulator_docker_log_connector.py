...
class EmulatorDockerLogConnector:
    ...
    def start(self):
        self.consumer_log = self.pool_log.map_async(
            __log_consumer__, ((self.log_queue, self.queue_out),)
        )
...
def __log_consumer__(args):
    ...
    while True:
        ...
        if my_clai:
            ...
            read(
                socket,
                lambda chunk: queue_out.put(
                    DockerReply(docker_reply="log", message=chunk)
                ),
            )
