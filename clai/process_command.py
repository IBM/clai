...
from clai.server.clai_message_builder import (
    create_message_help,
    create_message_server_runing,
)
    ...
    while True:
        ...
        if is_explain_command(command_input):
            print(
                Colorize()
                .warning()
                .append(f"Description: {command_to_execute.description}")
                .to_console()
            )
...
def process_command_from_user(command_id, user_name, command_to_check):
    command_to_execute = send_command(
        command_id=command_id, user_name=user_name, command_to_check=command_to_check
    )
...
def process_command(command_id, user_name, command_to_check):
    ...
    if command_to_check == COMMAND_START_SERVER:
        ...
        command_accepted_by_the_user, pending_commands = process_command_from_user(
            command_id, user_name, command_to_check
        )
