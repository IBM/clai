...
class ClaiUnselectCommandRunner(CommandRunner):
    ...
    def execute(self, state: State) -> Action:
        plugin_to_select = state.command.replace(
            f"{self.UNSELECT_DIRECTIVE}", ""
        ).strip()

        plugin_to_select = extract_quoted_agent_name(plugin_to_select)

        selected = self.agent_datasource.unselect_plugin(
        ...
        if selected:
            ...
            if (
                plugins_config.selected is not None
                and selected.name in plugins_config.selected
            ):
