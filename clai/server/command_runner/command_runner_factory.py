...
from clai.server.command_runner.clai_delegate_to_agent_command_runner import (
    ClaiDelegateToAgentCommandRunner,
)
...
from clai.server.command_runner.clai_last_info_command_runner import (
    ClaiLastInfoCommandRunner,
)
from clai.server.command_runner.clai_orchestrate_command_runner import (
    ClaiOrchestrateCommandRunner,
)
from clai.server.command_runner.clai_plugins_command_runner import (
    ClaiPluginsCommandRunner,
)
from clai.server.command_runner.clai_power_command_runner import ClaiPowerCommandRunner
from clai.server.command_runner.clai_power_disable_command_runner import (
    ClaiPowerDisableCommandRunner,
)
from clai.server.command_runner.clai_reload_command_runner import (
    ClaiReloadCommandRunner,
)
from clai.server.command_runner.clai_select_command_runner import (
    ClaiSelectCommandRunner,
)
from clai.server.command_runner.clai_unselect_command_runner import (
    ClaiUnselectCommandRunner,
)
...
class CommandRunnerFactory:
    ...
    def provide_command_runner(
        self, command: str, selected_agent: AgentRunner
    ) -> CommandRunner:
    ...
    def provide_post_command_runner(
        self, command: str, selected_agent: AgentRunner
    ) -> PostCommandRunner:
        if command.startswith(CLAI_COMMAND_NAME):
            clai_command_name = command.replace(CLAI_COMMAND_NAME, "", 1).strip()
            return self.__get_clai_post_command_runner(
                clai_command_name, selected_agent
            )

        return AgentCommandRunner(selected_agent, self.server_status_datasource)

    def __get_clai_post_command_runner(
        self, clai_command_name: str, selected_agent: AgentRunner
    ) -> PostCommandRunner:
        clai_post_commands_names = self.clai_post_commands.keys()

        commands_filtered = filter(
            clai_command_name.startswith, clai_post_commands_names
        )
    ...
    def __get_clai_command_runner(
        ...
        return ClaiDelegateToAgentCommandRunner(
            AgentCommandRunner(
                selected_agent, self.server_status_datasource, ignore_threshold=True
            )
        )
