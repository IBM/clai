    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.state["current_intent"] == "commit":
            ...
            if not self.state["ready_flag"]:
                ...
                temp_description = "Ready to {}. Press execute to continue.".format(
                    self.state["current_intent"]
                )

                return [
                    Action(
                        suggested_command="git status | tee {}".format(
                            _path_to_log_file
                        ),
                        execute=True,
                        description=None,
        ...
        if command == "execute":
            ...
            respond = Action(
                suggested_command='git commit -m "{}" -m "{}";'.format(
        ...
        if self.state["current_intent"] == "comment":
            ...
            ping_github = json.loads(
                self.gh_session.post(comment_url, json=payload).text
            )
            return Action(
                suggested_command=NOOP_COMMAND,
                execute=True,
