    ...
    def post_execute(self, state: State) -> Action:
        ...
        if bpx_matches is not None:
            ...
            result: CompletedProcess = subprocess.run(
                ["bpxmtext", reason_code], stdout=subprocess.PIPE
            )
        ...
        if not helpWasFound:
            ...
            description = (
                Colorize()
                .emoji(Colorize.EMOJI_ROBOT)
                .append(f"I couldn't find any help for message code '{msgid}'\n")
                .info()
                .to_console()
            )

        return Action(
            suggested_command=suggested_command, description=description, confidence=1.0
        )
