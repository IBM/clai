...
class KubeTracker:
    def __init__(self):
        ...
        self.template = open(
            _path_to_problem_template, mode="r", encoding="utf-8-sig"
        ).read()
...
class KubeExe(KubeTracker):
    ...
    def dockerfile_read(self):
        ...
        self.local_port = (
            re.findall(r"EXPOSE\s[0-9]+", dockerfile_contents)[0]
            .strip()
            .split(" ")[-1]
            .strip()
        )
    ...
    def docker_run(self):
        command = "docker run -i -p {}:{} -d {}:{}".format(
            self.host_port, self.local_port, self.name, self.tag
        )
    ...
    def docker_tag_for_ibmcloud(self):
        ...
        command = "docker tag {}:{} us.icr.io/{}/{}:{}".format(
            self.name, self.tag, self.namespace, self.name, self.tag
        )
        return Action(suggested_command=command, confidence=1.0, execute=False)
    ...
    def docker_push(self):
        ...
        command = "docker push us.icr.io/{}/{}:{}".format(
            self.namespace, self.name, self.tag
        )
        return Action(suggested_command=command, confidence=1.0, execute=False)
    ...
    def ibmcloud_delete_image(self):
        command = "ibmcloud cr image-rm us.icr.io/{}/".format(
            self.namespace, self.image_to_remove
        )
        return Action(suggested_command=command, confidence=1.0)
    ...
    def build_yaml(self):
        ...
        app_yaml = (
            app_yaml.replace("{name}", self.name)
    ...
    def get_set_cluster_config(self):
        ...
        command = 'ibmcloud ks cluster-config {} | grep -e "export" | echo'.format(
            self.cluster_name
        )
        return Action(suggested_command=command, confidence=1.0)
