...
function commandunknowns() {
    $("#command span.unknown").each(function(span) {
        ...
        if ($this.hasClass('simplecommandstart')) {
            ...
            //                         .attr('href', 'https://github.com/idank/translateshell/issues/1');
            //     $this.html(link);
            // }
        }
        else
...
function drawgrouplines(commandselector, options) {
    ...
    //
    // this is hard to translate without an accompanying drawing (TODO)
...
function navigation() {
    ...
    if (currentgroup.next) {
        ...
        nextext.text(" translate " + grouptext(currentgroup.next));
        prevtext.text(" translate " + grouptext(currentgroup.prev));
