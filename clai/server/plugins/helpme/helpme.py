...
class HelpMeAgent(Agent):
    ...
    def compute_confidence(self, query, forum, manpage):
        ...
        """
        query_forum_similarity = self.compute_simple_token_similarity(
            query, forum[0]["Content"]
        )
        forum_manpage_similarity = self.compute_simple_token_similarity(
            forum[0]["Answer"], manpage
        )
            ...
            for variant in variants:
                ...
                if variant is not None:
                    ...
                    data = self.store.search(
                        state.stderr, service=provider, size=1, searchType=variant
                    )
                else:
